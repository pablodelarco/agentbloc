...
> Pattern for routing agent failures + critical errors through a
> dedicated Telegram thread. Ships INSIDE the emitted spec folder as
> `agents/<id>/escalation.md` per-agent prose plus the optional
> `runtime/reference-impl/escalation-router.sh` primitive. AgentBloc
