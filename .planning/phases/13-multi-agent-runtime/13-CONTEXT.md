...
- `.claude/agents/runtime-engine.md` (new) , Claude Code subagent that materializes wake.md files per agent, registers crontab entries (or writes `crontab.applied` manifest), emits n8n route YAML stubs for the user's n8n instance, and extends registry.yaml with the `runtime` top-level block. `context: fork` + scoped tools (Read/Grep/Glob/Write/Edit; Bash narrowed to `crontab -` (stdin install), `crontab -l`, `shasum -a 256`, `claude agents list`, `claude mcp list`; NO WebFetch, NO other MCPs). `crontab -e` is EXPLICITLY DISALLOWED because it launches an interactive editor (`$EDITOR`) that would hang a forked subagent; scripted install must use stdin form `(crontab -l 2>/dev/null; cat .agentbloc/runtime/crontab.applied) | crontab -`. Parallels deploy-engine (Phase 12) / designer-agent (Phase 9) / browser-discovery (Phase 11).
...
- **Inherited D-21 (Phase 9):** Subagent with `context: fork`, scoped tools, Bash narrowly allow-listed. runtime-engine gets Read/Grep/Glob/Write/Edit + narrowed Bash allow-list (`crontab -` (stdin install), `crontab -l`, `shasum -a 256`, `claude agents list`, `claude mcp list`) + NO WebFetch + NO other MCPs. This is a narrower Bash surface than deploy-engine (D-67) because runtime-engine mutates crontab (a system-level resource); `crontab -e` is EXPLICITLY DISALLOWED (interactive editor hangs a forked subagent); the allow-list is locked in the subagent's `<write_constraint>` XML block.
...
- **Inherited Phase 12 D-67 (deploy-engine narrow Bash):** Narrow Bash allow-list for subagents that touch system state. Phase 13 tightens: runtime-engine's Bash allow-list is explicitly enumerated (`crontab -` (stdin install), `crontab -l`, `shasum -a 256`, `claude agents list`, `claude mcp list`). `crontab -e` is EXPLICITLY DISALLOWED because it launches an interactive editor that hangs a forked subagent. See D-80 below for the full list + stdin-install pattern + rejection of broad-Bash alternatives.
...
- **D-73 (Wake-job template is a 6-section markdown file consumed by `claude -p`, materialized per agent per trigger type, three template variants dispatched on trigger.type):** Every deployed agent gets one or more wake.md files at `.agentbloc/agents/<agent-id>/wake-<slug>.md`. The wake.md file is the literal prompt `claude -p` receives.

  **Naming convention (bounded + collision-safe):**
  - `wake-cron.md` , one per agent; only one cron schedule per agent per PDF page 3 (trigger.schedule is a single string, not an array).
  - `wake-webhook-<source>-<event-slug>.md` , one per (source, event) pair; e.g., `wake-webhook-bbva-payment-received.md`, `wake-webhook-timer-overdue-7d.md`, `wake-webhook-telegram-tenant-message.md`. `<source>` comes from the n8n route's `trigger.source` field (enum: gmail / plaid / bbva / google-calendar / telegram / form / custom-<name>); `<event-slug>` is a kebab-case derivation of the agent-profile's `triggers[].event` value.
  - `wake-inter.md` , exactly one per agent (agents share a single inter-agent handler that dispatches internally by incoming `message.type` per the registry-declared message contract; no split-by-caller because multiple callers with the same message type share handler logic).
  - `wake-manual.md` , OPTIONAL; generated only if the agent profile declares a `manual` trigger (rare; typically for dev-only one-off invocations).

  The result: for the 3-agent Arco Rooms team with triggers per PDF page 3 (Gestor Cobros = 1 cron + 2 webhooks, Recepcionista = 2 webhooks + 1 inter, Gestor Documental = 1 cron), the file count is 3 + 3 + 1 = 7 wake.md files total , bounded and auditable. runtime-engine materializes them deterministically from agent-profiles.yaml.triggers[] via D-73 Option D.

  **Structure (6 sections, fixed order, mandatory):**
...
- **D-80 (runtime-engine subagent: `.claude/agents/runtime-engine.md`, `context: fork`, tools Read/Grep/Glob/Write/Edit + narrowed Bash allow-list; parallels Phase 12 deploy-engine):** runtime-engine runs as the final step of `deploy-protocol.md` (after deploy-engine completes successfully). Responsibilities:
  ...
  4. Diff `.agentbloc/deploy/crontab.proposed` against current `crontab -l`; if non-empty diff, present to user; on approval, install via stdin form: `(crontab -l 2>/dev/null; cat .agentbloc/runtime/crontab.applied) | crontab -`. NEVER use `crontab -e` (interactive editor; hangs the forked subagent).
  ...
  **Tool scope (written into the subagent's `<write_constraint>` XML block):**
  ...
  - **Bash narrow allow-list:** `crontab -` (stdin install), `crontab -l`, `shasum -a 256`, `claude agents list`, `claude mcp list`. NO `crontab -e` (interactive editor hangs the subagent), NO `bash -c`, NO `sh`, NO `curl`, NO `rm -rf`, NO wildcards. Every invocation must match one of these 5 command prefixes exactly. The stdin-install pattern is: `(crontab -l 2>/dev/null; cat .agentbloc/runtime/crontab.applied) | crontab -` -- produces the same result as editing the crontab, but is scripted, deterministic, and survives in a forked context.
...
- **Phase 12 deploy-engine's Bash allow-list** (`crontab -l`, `shasum -a 256`, `claude agents list`, `claude mcp list`) , runtime-engine adopts verbatim + adds `crontab -` (stdin install). The verification-read commands are shared; the mutation command (`crontab -`, stdin form) is unique to runtime-engine per D-80. `crontab -e` (interactive editor) is explicitly NOT on the allow-list because it would hang a forked subagent waiting for `$EDITOR`.
