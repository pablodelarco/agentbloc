...
**Two-round user-review revision (2026-04-24):**

- **Round 1:** Original auto-decision put all 4 per-agent files under `.claude/agents/<agent-id>/` as a single co-located directory. Pablo flagged the REQUIREMENTS-literal tension and asked for an architect-grade recommendation. First revised decision (Option C below) split into three sub-decisions with namespace hygiene as the organizing principle: SKILL.md at project-root `skills/<agent-id>/`, memory + registry at `.agentbloc/agents/`.
- **Round 2 (post-research):** 12-RESEARCH.md topic 1 surfaced that "ClaudeClaw" names multiple distinct projects with different conventions (robonuggets blueprint, htdocs.dev TS gateway, OpenClaw) AND project-root `skills/` is NOT a Claude Code native skill discovery path. Pablo confirmed he uses ClaudeClaw (OpenClaw-based), which reads `.claude/skills/` per Claude Code native convention. The Round-1 rationale "honors DEPLOY-01 literal for ClaudeClaw compatibility" was therefore weak: ClaudeClaw-compatibility is actually MET by using `.claude/skills/`, not by using project-root `skills/`. Pablo asked "what's best per AI agent architect criterion?" and approved the architect-recommended path `.claude/skills/<agent-id>/SKILL.md` which is Claude Code native + runtime-agnostic + still OpenClaw-compatible. D-59a thus shifts from literal-honor to literal-override on architectural grounds (Claude Code native skill discovery).
...
| B. Original D-59 (DEPLOY-01 co-location) | `.claude/agents/<id>/SKILL.md` | `.claude/agents/<id>/` | `.claude/agents/registry.yaml` | Rejected Round-1. Everything inside Claude Code reserved namespace; mixes developer tooling with customer runtime; violates stable-vs-mutable split. |
| C. Round-1 double override | `skills/<id>/SKILL.md` (project root) | `.agentbloc/agents/<id>/` | `.agentbloc/agents/registry.yaml` | Rejected Round-2. Project-root `skills/` is not Claude Code native skill discovery path; ClaudeClaw-compatibility rationale could not be externally verified (no public README defines `skills/`-at-root as a ClaudeClaw contract). |
...
| E. Triple override with Claude Code native skill path | `.claude/skills/<id>/SKILL.md` | `.agentbloc/agents/<id>/` | `.agentbloc/agents/registry.yaml` | **Selected Round-2 (D-59a + D-59b + D-59c).** Deployed agents become peer skills to agentbloc and mcp-builder; runtime-agnostic; OpenClaw-compatible per native `.claude/skills/` convention; keeps stable-vs-mutable split from Round-1. |

**Auto-selected (Round-2 revision, user-approved):** Option E , triple-override with Claude Code native skill discovery.

- **D-59a:** Deployed agent SKILL.md at `.claude/skills/<agent-id>/SKILL.md` (DEPLOY-01 literal OVERRIDDEN for Claude Code native skill discovery; deployed agents become peer skills to agentbloc and mcp-builder; runtime-agnostic; OpenClaw/ClaudeClaw compatible per native convention).
...
**Notes:** The Round-2 revision is a superset improvement over Round-1. Round-1 correctly identified the stable-vs-mutable split as the architectural principle; Round-2 additionally fixes the SKILL.md path to align with Claude Code native skill discovery. All three REQUIREMENTS.md literals are now overridden, but each override is defensible: D-59a by Claude Code native convention (not by arbitrary preference), D-59b by namespace hygiene, D-59c by co-location with data. Plan 12-01 header MUST document all three overrides with their specific architectural principle so Phase 16 audit readers see "REQUIREMENTS literal says X; Phase 12 ships Y; here is why Y is correct."

**Files consulted:** REQUIREMENTS.md DEPLOY-01 + MEM-01 + DEPLOY-05, existing `.claude/agents/designer-agent.md` and `browser-discovery.md` shape, `.claude/skills/agentbloc/SKILL.md` and `.claude/skills/mcp-builder/SKILL.md` as peer-skill precedents, Phase 11 `.agentbloc/discovery/` convention for customer-state namespace, 12-RESEARCH.md topic 1 (ClaudeClaw naming ambiguity + OpenClaw native-convention confirmation).

**Prior decisions applied:** Phase 8 D-15 (`.agentbloc/` for customer-mutable state, `.claude/` for Claude Code tooling). Phase 11 `.agentbloc/discovery/` convention extends to `.agentbloc/agents/` and `.agentbloc/deploy/`. Claude Code native skill discovery path `.claude/skills/<name>/SKILL.md` already used by AgentBloc itself (`.claude/skills/agentbloc/SKILL.md`) and mcp-builder; deployed agents are peer skills in the same registry.
...
- **D-59 split into D-59a / D-59b / D-59c (two-round user-approved revision, 2026-04-24):** REQUIREMENTS.md is internally inconsistent (DEPLOY-01 literal `skills/{agent-id}/` vs MEM-01 literal `.claude/agents/{agent-id}/` vs DEPLOY-05 literal `.claude/agents/registry.yaml`). Round-1 resolution honored DEPLOY-01 (skills/ at project root) and overrode MEM-01 + DEPLOY-05 for namespace hygiene. Round-2 (post-research) surfaced that project-root `skills/` is not a Claude Code native skill discovery path and that ClaudeClaw-compatibility rationale could not be externally verified; Pablo asked for the architect-grade criterion and approved the triple-override: D-59a moves to `.claude/skills/<agent-id>/SKILL.md` (Claude Code native skill discovery path; peer to agentbloc and mcp-builder; runtime-agnostic; OpenClaw-compatible), D-59b at `.agentbloc/agents/<agent-id>/` (namespace hygiene), D-59c at `.agentbloc/agents/registry.yaml` (co-location). Plan 12-01 header MUST surface all three overrides with their specific architectural principles (native convention + namespace hygiene + stable-vs-mutable split) so Phase 16 audit readers see "REQUIREMENTS says X; Phase 12 ships Y; here is why Y is correct" for each.

- **D-60 extended with RFC 8785 canonicalization (2026-04-24, post-research):** 12-RESEARCH.md topic 2 flagged that SHA256 over body with timestamp masking alone does not handle JSON key-order variance. Machine-written JSON artifacts (state.json, last-run.json, DEPLOY_HISTORY.jsonl lines, `.mcp.json` delta) must be re-serialized via RFC 8785 JSON Canonicalization Scheme before hashing to avoid false-positive diffs when the writer reorders keys. Markdown artifacts are unaffected. Low-effort extension; high-determinism payoff for Phase 16 golden-file tests.

- **D-62 split into three autonomy-level templates (2026-04-24, post-research):** 12-RESEARCH.md topic 3 flagged that a single Jinja-lite template with `{% if autonomy == "..." %}` conditional blocks is fragile under Claude-in-context substitution (escaping + conditional-branch + loop-rendering all introduce token-level variance). Remediation: three separate templates at `.claude/skills/agentbloc/templates/deployed-agent-skill-{full,semi,supervised}.md.tmpl`; deploy-engine picks one based on `agent.autonomy_level` (one-line routing, zero in-template branching). `{{agent.tools}}` bullet-list requires pre-computation from integration-manifest before substitution (deploy-engine renders the list as a string; template sees already-rendered text).

- **D-69 tightened with canonical `tools/list` and retry policy (2026-04-24, post-research):** 12-RESEARCH.md topic 5 confirmed `tools/list` as the canonical MCP health-check method and surfaced the ecosystem norm of 5s (warm) + 10s (cold-start) + retry=3 with exponential backoff. D-69 check 2 updates to call `tools/list` per MCP server with the split timeout policy.
